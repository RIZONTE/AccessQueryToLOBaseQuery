...
access_db_path = os.path.join(inputFiles, r"Сверка — копия.accdb")                              # путь до файла access
meta_output_file = os.path.join(outFiles, r"access_meta.txt")                                   # путь до выходного файла с метаданными таблиц
queries_out_file = os.path.join(outFiles, r"queries.txt")                                       # путь до выходного файла с запросами
...
for i in range(len(excel_files)):
    csv_output = outFiles +"/" + table_names[i] + ".csv"
